...
%EOP_SimulateGame
%This is a function which takes the part of the working script. Hence all
%non-function code is simply copy-pased from the working script, and should
%be kept that way to maintain consistency 
...
%% GLOBAL PARAMETERS (ASSIGNED FROM FUNCTION)
%Randomization
%seed = seed; 
    rng(seed);
...
% T = T;
% N = N; %We simulate a population of N. 
% 
% Population means
...
figure;  set(gcf, 'Position', [80,80, 800, 800])
        ...
        lgd = legend('Principal (FB)','Principal', 'Average agent (FB)', 'Average agent',...
            'Location', 'northwest');
        lgd.FontSize = 10;
        lgd.FontName = 'CMU Serif';
        lgd_pos = get(lgd,'position'); 
        ...
        set(fig2, {'Marker'}, {'none';'o';'d';'square'}); 
        ...
        set(fig3, {'Color'}, {[0.4940    0.1840    0.5560];[0   0.4470    0.7410]...
                    ;[  0.4660    0.6740    0.1880];[0.8500    0.3250    0.0980]});
        set(fig3, {'Marker'}, {'none';'o';'d';'square'}); 
